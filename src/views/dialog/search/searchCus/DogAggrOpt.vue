    ...
    <v-card>
      ...
      <v-card-text>
        ...
        <v-row no-gutters style="height: 60px">
          <v-spacer />
          <v-col cols="12" md="11">
            <div class="title-medium">강아지의 공격성</div>
          </v-col>
        </v-row>
        ...
        <v-row justify="center" no-gutters style="height: 100px">
          <v-select
            v-model="selected"
            :items="aggrs"
            label="수용 가능한 강아지의 공격성 정도를 선택하세요(중복가능)"></v-select>
        </v-row>
        <!-- 칩 -->
...
export default {
  ...
  data() {
    return {
      selectedList: [],
      selected: null,
      aggrs: ['매우 심함', '약간 심함', '보통', '약간 양호', '매우 양호'],
    };
  },
  methods: {
    clickClose() {
    ...
    cancelChip(one) {
      console.log('뭐야');
      console.log('4 : ', this.selectedList);
      for (let i = 0; i < this.selectedList.length; i++) {
        if (this.selectedList[i] === one) {
          this.selectedList.splice(i, 1);
          console.log('5 : ', this.selectedList);
        }
      }
    },
    clickSubmit() {
      ...
      let formatList = [];
      for (let i = 0; i < this.selectedList.length; i++) {
        formatList.push(format.convertDogAggr(this.selectedList[i]));
      }
      this.$store.commit('setDogAggrs', formatList);
    },
    clickCancel() {
      this.$store.commit('setOpen', false); // 창 닫기
    },
